...
const DIFFICULTY_LEVELS = ['EMT-Basic', 'Paramedic', 'Critical Care'];
const GAME_MODES = ['Free Practice', 'Timed Challenge'];

const LEAD_PLACEMENTS = {
  'V1': { 
    x: 52, y: 35, 
...
class AirwayGame {
  ...
  start(difficulty = 'all') {
    this.difficulty = difficulty;
    this.selectRandomScenario();
  }

  selectRandomScenario() {
    let available = AIRWAY_SCENARIOS;
    if (this.difficulty !== 'all') {
      available = AIRWAY_SCENARIOS.filter(s => s.difficulty === this.difficulty);
    }
  ...
  render() {
    ...
    if (!this.currentScenario) {
      ...
      this.renderPresentation(container);
    } else if (this.currentStep <= this.currentScenario.decisionTree.length) {
      this.renderDecision(container);
    } else {
  ...
  renderMenu(container) {
    container.innerHTML = `
      <div style="max-width: 800px; margin: 0 auto; text-align: center;">
        <h2 style="color: var(--primary); margin-bottom: 2rem;">Airway Management Decision Tree</h2>
        
        <p style="margin-bottom: 2rem; color: var(--text-muted);">
          Work through real-world airway scenarios. Make critical decisions under pressure.
        </p>

        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
          <button onclick="window.airwayGame.start('all')" class="btn btn-full" style="padding: 1.5rem;">
            <strong>All Levels</strong><br><small>Mixed scenarios</small>
          </button>
          <button onclick="window.airwayGame.start('EMT-Basic')" class="btn btn-full" style="padding: 1.5rem;">
            <strong>EMT-Basic</strong><br><small>BLS airways</small>
          </button>
          <button onclick="window.airwayGame.start('Paramedic')" class="btn btn-full" style="padding: 1.5rem;">
            <strong>Paramedic</strong><br><small>ALS airways</small>
          </button>
          <button onclick="window.airwayGame.start('Critical Care')" class="btn btn-full" style="padding: 1.5rem;">
            <strong>Critical Care</strong><br><small>Complex airways</small>
          </button>
        </div>

        <div style="background: rgba(33, 128, 141, 0.1); padding: 1.5rem; border-radius: 8px; text-align: left;">
          <h4 style="margin-bottom: 1rem;">You'll Learn:</h4>
          <ul style="color: var(--text-muted); line-height: 1.8;">
            <li>Rapid airway assessment and decision-making</li>
            <li>When to use basic vs advanced interventions</li>
            <li>Recognition of "can't intubate, can't oxygenate" scenarios</li>
            <li>Proper escalation pathways for failing airways</li>
          </ul>
        </div>
      </div>
    `;
    ...
    container.innerHTML = `
      <div style="max-width: 900px; margin: 0 auto;">
        <div style="display: flex; justify-content: space-between; margin-bottom: 1rem;">
          <button onclick="window.airwayGame.currentScenario = null; window.airwayGame.render();" class="btn">← Back</button>
          <span style="background: var(--primary); color: white; padding: 0.5rem 1rem; border-radius: 4px; font-size: 0.9rem;">
            ${s.difficulty}
          </span>
        </div>
        ...
        <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 2rem;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
            <div>
              <h4 style="margin-bottom: 0.5rem;">Patient Info:</h4>
              <p><strong>Age:</strong> ${p.age} years</p>
              <p><strong>Gender:</strong> ${p.gender}</p>
              <p><strong>Chief Complaint:</strong> ${p.chiefComplaint}</p>
              <p style="margin-top: 1rem;"><strong>Scene:</strong> ${p.scene}</p>
            </div>

            <div>
              <h4 style="margin-bottom: 0.5rem;">Vitals:</h4>
              <p><strong>GCS:</strong> ${p.vitals.gcs}</p>
              <p><strong>RR:</strong> ${p.vitals.rr}</p>
              <p><strong>HR:</strong> ${p.vitals.hr}</p>
              <p><strong>BP:</strong> ${p.vitals.bp}</p>
              <p><strong>SpO2:</strong> ${p.vitals.spo2}%</p>
            </div>
          </div>
          ...
          <div style="margin-top: 2rem; padding-top: 2rem; border-top: 1px solid var(--border);">
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
              <div>
                <h4 style="margin-bottom: 0.5rem; color: var(--accent);">Airway:</h4>
                <p style="color: var(--text-muted);">${p.airway}</p>
              </div>
              <div>
                <h4 style="margin-bottom: 0.5rem; color: var(--accent);">Breathing:</h4>
                <p style="color: var(--text-muted);">${p.breathing}</p>
              </div>
              <div>
                <h4 style="margin-bottom: 0.5rem; color: var(--accent);">Circulation:</h4>
                <p style="color: var(--text-muted);">${p.skin}</p>
              </div>
            </div>
          </div>
        </div>

        <button onclick="window.airwayGame.currentStep = 1; window.airwayGame.render();" class="btn btn-full" style="padding: 1rem; font-size: 1.1rem;">
          Begin Scenario →
        </button>
      </div>
    `;
  ...
  renderDecision(container) {
    const decision = this.currentScenario.decisionTree[this.currentStep - 1];
    const totalSteps = this.currentScenario.decisionTree.length;

    container.innerHTML = `
      <div style="max-width: 800px; margin: 0 auto;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
          <h3 style="color: var(--primary);">${this.currentScenario.title}</h3>
          <p style="color: var(--text-muted);">Step ${this.currentStep} of ${totalSteps}</p>
        </div>

        <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 2rem;">
          <p style="font-size: 1.2rem; font-weight: 600; margin-bottom: 2rem;">${decision.question}</p>

          <div style="display: grid; gap: 1rem;">
            ${decision.options.map((opt, i) => `
              <button onclick="window.airwayGame.checkAnswer(${i})" class="answer-btn" style="text-align: left; padding: 1.5rem;">
                ${opt.text}
              </button>
            `).join('')}
          </div>
  ...
  checkAnswer(index) {
    const decision = this.currentScenario.decisionTree[this.currentStep - 1];
    const selected = decision.options[index];
    ...
    feedback.innerHTML = `
      <div style="background: ${selected.correct ? 'rgba(33, 128, 141, 0.1)' : 'rgba(192, 21, 47, 0.1)'}; 
                  padding: 1.5rem; border-radius: 8px; border-left: 4px solid ${selected.correct ? 'var(--primary)' : 'var(--accent)'};">
        <p style="font-size: 1.2rem; margin-bottom: 1rem;">
          <strong>${selected.correct ? '✓ Correct!' : '✗ Incorrect'}</strong>
        </p>
        <p style="margin-bottom: 1rem;">${selected.feedback}</p>
        ${!selected.correct ? `<p style="padding-top: 1rem; border-top: 1px solid var(--border);"><strong>Correct answer:</strong> ${correct.text}</p>` : ''}
        <button onclick="window.airwayGame.nextStep()" class="btn btn-full" style="margin-top: 1.5rem;">
          Continue →
        </button>
      </div>
    `;
  ...
  renderComplete(container) {
    ...
    container.innerHTML = `
      <div style="max-width: 700px; margin: 0 auto; text-align: center;">
        <h2 style="color: var(--primary); margin-bottom: 2rem;">Scenario Complete!</h2>

        <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 2rem;">
          <h3 style="margin-bottom: 1rem;">${this.currentScenario.title}</h3>
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem; margin: 2rem 0;">
            <div>
              ...
              <p style="color: var(--text-muted);">Correct Decisions</p>
            </div>
            <div>
        ...
        <div style="background: rgba(33, 128, 141, 0.1); padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; text-align: left;">
          <h4 style="margin-bottom: 1rem;">💎 Clinical Pearls:</h4>
          <ul style="color: var(--text-muted); line-height: 1.8;">
            ${this.currentScenario.clinicalPearls.map(pearl => `<li>${pearl}</li>`).join('')}
          </ul>
        </div>

        <div style="display: flex; gap: 1rem; justify-content: center;">
          <button onclick="window.airwayGame.selectRandomScenario()" class="btn btn-full">Next Scenario</button>
          <button onclick="window.airwayGame.currentScenario = null; window.airwayGame.render();" class="btn">Main Menu</button>
        </div>
      </div>
    `;
  }
...
class SAMPLEOPQRSTGame {
  constructor() {
    this.currentScenario = null;
    this.askedQuestions = [];
    this.missedCritical = [];
    this.score = 0;
    this.mode = 'guided'; // 'guided' or 'free'
  }
  ...
  selectScenario() {
    this.currentScenario = SAMPLE_OPQRST_SCENARIOS[Math.floor(Math.random() * SAMPLE_OPQRST_SCENARIOS.length)];
    this.askedQuestions = [];
    this.missedCritical = [];
    this.score = 0;
    this.render();
  ...
  render() {
    ...
    if (!this.currentScenario) {
      ...
      this.renderInterview(container);
    }
  }
  ...
  renderMenu(container) {
    container.innerHTML = `
      <div style="max-width: 800px; margin: 0 auto; text-align: center;">
        <h2 style="color: var(--primary); margin-bottom: 2rem;">SAMPLE / OPQRST History Taking</h2>
        
        <p style="margin-bottom: 2rem; color: var(--text-muted); line-height: 1.6;">
          Practice patient interviews and learn to gather complete, relevant histories. 
          Master the mnemonics that guide critical thinking in the field.
        </p>

        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
          <div onclick="window.sampleGame.start('guided')" style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 2px solid var(--border); cursor: pointer; transition: all 0.2s;"
               onmouseover="this.style.borderColor='var(--primary)'" 
               onmouseout="this.style.borderColor='var(--border)'">
            <h3 style="color: var(--primary); margin-bottom: 1rem;">Guided Mode</h3>
            <p style="color: var(--text-muted);">Prompts for each element of SAMPLE and OPQRST</p>
          </div>

          <div onclick="window.sampleGame.start('free')" style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 2px solid var(--border); cursor: pointer; transition: all 0.2s;"
               onmouseover="this.style.borderColor='var(--primary)'" 
               onmouseout="this.style.borderColor='var(--border)'">
            <h3 style="color: var(--primary); margin-bottom: 1rem;">Free Interview</h3>
            <p style="color: var(--text-muted);">Ask questions freely, get scored on completeness</p>
          </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; text-align: left;">
          <div style="background: var(--bg-surface); padding: 1.5rem; border-radius: 8px; border: 1px solid var(--border);">
            <h4 style="color: var(--primary); margin-bottom: 1rem;">OPQRST (Pain/Symptom)</h4>
            <ul style="color: var(--text-muted); line-height: 1.8;">
              <li><strong>O</strong>nset - When did it start?</li>
              <li><strong>P</strong>rovocation - What makes it better/worse?</li>
              <li><strong>Q</strong>uality - What does it feel like?</li>
              <li><strong>R</strong>adiation - Does it go anywhere?</li>
              <li><strong>S</strong>everity - How bad (1-10)?</li>
              <li><strong>T</strong>ime - How long? Constant/intermittent?</li>
            </ul>
          </div>

          <div style="background: var(--bg-surface); padding: 1.5rem; border-radius: 8px; border: 1px solid var(--border);">
            <h4 style="color: var(--primary); margin-bottom: 1rem;">SAMPLE (Medical Hx)</h4>
            <ul style="color: var(--text-muted); line-height: 1.8;">
              <li><strong>S</strong>igns/Symptoms - What else?</li>
              <li><strong>A</strong>llergies - Any medication allergies?</li>
              <li><strong>M</strong>edications - What meds do you take?</li>
              <li><strong>P</strong>ast history - Medical conditions?</li>
              <li><strong>L</strong>ast intake - When did you last eat/drink?</li>
              <li><strong>E</strong>vents - What led up to this?</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  renderInterview(container) {
    ...
    const totalQuestions = Object.keys(this.currentScenario.answers).length;
    const asked = this.askedQuestions.length;

    container.innerHTML = `
      <div style="max-width: 1000px; margin: 0 auto;">
        <div style="display: flex; justify-content: space-between; margin-bottom: 1rem;">
          <button onclick="window.sampleGame.currentScenario = null; window.sampleGame.render();" class="btn">← Back</button>
          <div style="text-align: right;">
            <p><strong>Progress:</strong> ${asked}/${totalQuestions} questions asked</p>
            <p style="color: var(--text-muted); font-size: 0.9rem;">${this.currentScenario.difficulty}</p>
          </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
          <!-- Patient Interview -->
          <div>
            <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 1rem;">
              <h3 style="color: var(--primary); margin-bottom: 1rem;">Patient: ${p.name}</h3>
              <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 1rem;">
                <p><strong>Age:</strong> ${p.age}</p>
                <p><strong>Gender:</strong> ${p.gender}</p>
              </div>
              <p style="margin-bottom: 1rem;"><strong>Chief Complaint:</strong> "${p.chiefComplaint}"</p>
              <p style="color: var(--text-muted);"><strong>Appearance:</strong> ${p.appearance}</p>
            </div>

            <!-- Question Buttons -->
            <div style="background: var(--bg-surface); padding: 1.5rem; border-radius: 8px; border: 1px solid var(--border);">
              <h4 style="margin-bottom: 1rem;">Select Your Question:</h4>
              <div style="display: grid; gap: 0.5rem;">
                ${this.renderQuestionButtons()}
              </div>
              
              ${asked >= totalQuestions ? `
                <button onclick="window.sampleGame.showResults()" class="btn btn-full" style="margin-top: 1.5rem;">
                  Complete Interview →
                </button>
              ` : ''}
            </div>
          ...
          <!-- Interview Notes -->
          <div>
            <div style="background: var(--bg-surface); padding: 1.5rem; border-radius: 8px; border: 1px solid var(--border); max-height: 600px; overflow-y: auto;">
              <h4 style="margin-bottom: 1rem; position: sticky; top: 0; background: var(--bg-surface); padding-bottom: 0.5rem;">📝 Interview Notes</h4>
              ${this.askedQuestions.length === 0 ? 
                '<p style="color: var(--text-muted); font-style: italic;">No questions asked yet...</p>' : 
                this.renderNotes()
              }
            </div>
          </div>
        ...
        <div id="patient-response" style="margin-top: 1rem;"></div>
      </div>
    `;
  }
  ...
  renderQuestionButtons() {
    const questions = {
      ...
      pastHistory: 'Any past medical problems?',
      lastIntake: 'When did you last eat or drink?',
      events: 'What were you doing when this started?'
    };

    let html = '';
    for (const [key, question] of Object.entries(questions)) {
      const asked = this.askedQuestions.includes(key);
      const mnemonic = this.getMnemonic(key);
      
      html += `
        <button onclick="window.sampleGame.askQuestion('${key}')" 
                class="answer-btn ${asked ? 'disabled' : ''}" 
                style="text-align: left; padding: 1rem; ${asked ? 'opacity: 0.5;' : ''}"
                ${asked ? 'disabled' : ''}>
          <span style="display: inline-block; width: 25px; font-weight: bold; color: var(--primary);">${mnemonic}</span>
          ${question}
          ${asked ? '<span style="float: right;">✓</span>' : ''}
        </button>
      `;
    }
    return html;
  ...
  getMnemonic(key) {
    const map = {
      onset: 'O', provocation: 'P', quality: 'Q', radiation: 'R', severity: 'S', time: 'T',
      signs: 'S', allergies: 'A', medications: 'M', pastHistory: 'P', lastIntake: 'L', events: 'E'
    };
    return map[key] || '';
  }
  ...
  renderNotes() {
    ...
    this.askedQuestions.forEach((key, index) => {
      const answer = this.currentScenario.answers[key];
      const mnemonic = this.getMnemonic(key);
      html += `
        <div style="margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border);">
          <p style="font-weight: 600; color: var(--primary); margin-bottom: 0.25rem;">
            ${mnemonic} - ${this.formatKey(key)}
          </p>
  ...
  formatKey(key) {
    const map = {
      onset: 'Onset', provocation: 'Provocation', quality: 'Quality', 
      radiation: 'Radiation', severity: 'Severity', time: 'Time',
      signs: 'Signs/Symptoms', allergies: 'Allergies', medications: 'Medications',
      pastHistory: 'Past History', lastIntake: 'Last Intake', events: 'Events'
    };
    return map[key] || key;
  }

  askQuestion(key) {
    if (this.askedQuestions.includes(key)) return;

    this.askedQuestions.push(key);
    const answer = this.currentScenario.answers[key];

    // Show patient response
    const responseDiv = document.getElementById('patient-response');
    responseDiv.innerHTML = `
      <div style="background: rgba(33, 128, 141, 0.1); padding: 1.5rem; border-radius: 8px; border-left: 4px solid var(--primary); animation: slideIn 0.3s;">
        <p style="font-weight: 600; margin-bottom: 0.5rem;">Patient Response:</p>
  ...
  showResults() {
    ...
    const totalQuestions = Object.keys(this.currentScenario.answers).length;
    const asked = this.askedQuestions.length;
    const completeness = Math.round((asked / totalQuestions) * 100);

    // Check for missed critical questions
    const criticalKeys = ['onset', 'severity', 'allergies', 'medications', 'pastHistory'];
    this.missedCritical = criticalKeys.filter(k => !this.askedQuestions.includes(k));

    container.innerHTML = `
      <div style="max-width: 800px; margin: 0 auto;">
        <h2 style="color: var(--primary); text-align: center; margin-bottom: 2rem;">Interview Complete</h2>
        ...
        <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 2rem;">
          ...
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem; margin: 2rem 0;">
            <div style="text-align: center;">
              <p style="font-size: 2rem; color: var(--primary); font-weight: bold;">${asked}/${totalQuestions}</p>
              <p style="color: var(--text-muted);">Questions Asked</p>
            </div>
            <div style="text-align: center;">
              <p style="font-size: 2rem; color: ${completeness >= 90 ? 'var(--primary)' : 'var(--accent)'}; font-weight: bold;">${completeness}%</p>
              <p style="color: var(--text-muted);">Completeness</p>
            </div>
          </div>

          ${this.missedCritical.length > 0 ? `
            <div style="background: rgba(192, 21, 47, 0.1); padding: 1rem; border-radius: 8px; border-left: 4px solid var(--accent); margin-top: 1rem;">
              <p style="font-weight: 600; margin-bottom: 0.5rem;">⚠️ Missed Critical Questions:</p>
              <ul style="color: var(--text-muted); margin-left: 1.5rem;">
                ${this.missedCritical.map(k => `<li>${this.formatKey(k)}</li>`).join('')}
              </ul>
            </div>
          ` : `
            <div style="background: rgba(33, 128, 141, 0.1); padding: 1rem; border-radius: 8px; border-left: 4px solid var(--primary);">
              <p style="font-weight: 600;">✓ All critical questions asked!</p>
            </div>
          `}
        </div>

        <div style="background: var(--bg-surface); padding: 2rem; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 2rem;">
          <h4 style="margin-bottom: 1rem;">Clinical Findings:</h4>
          <ul style="color: var(--text-muted); line-height: 1.8; margin-bottom: 1.5rem;">
            ${this.currentScenario.criticalFindings.map(f => `<li>${f}</li>`).join('')}
          </ul>

          <h4 style="margin-bottom: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">🚩 Red Flags:</h4>
          <ul style="color: var(--accent); line-height: 1.8; margin-bottom: 1.5rem;">
            ${this.currentScenario.redFlags.map(f => `<li>${f}</li>`).join('')}
          </ul>

          <div style="background: rgba(33, 128, 141, 0.1); padding: 1rem; border-radius: 8px;">
            <p style="font-weight: 600; margin-bottom: 0.5rem;">Working Diagnosis:</p>
            <p style="font-size: 1.1rem; color: var(--primary);">${this.currentScenario.suspectedDx}</p>
          </div>
        </div>

        <div style="display: flex; gap: 1rem; justify-content: center;">
          <button onclick="window.sampleGame.selectScenario()" class="btn btn-full">Next Patient</button>
          <button onclick="window.sampleGame.currentScenario = null; window.sampleGame.render();" class="btn">Main Menu</button>
        </div>
      </div>
    `;
  }
